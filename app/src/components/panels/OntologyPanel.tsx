...
// Mock recently activated concepts
...
// Radar chart for context vectors
    ...
    <div className="relative">
      <svg width={size} height={size} className="transform hover:scale-105 transition-transform duration-300">
        ...
        {points.map((p, i) => (
          ...
          >
            <title>{p.name}: {(p.value * 100).toFixed(0)}%</title>
          </motion.circle>
        ))}
      ...
      <div className="mt-2 space-y-1">
        {vectors.map((v) => (
          <div key={v.name} className="flex items-center gap-2 text-xs">
            ...
            <span className="ml-auto font-medium">{(v.value * 100).toFixed(0)}%</span>
          </div>
        ))}
      </div>
    </div>
